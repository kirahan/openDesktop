# OpenDesktop CLI 手册（`opd` / `yarn oc`）

> **依据**：CLI 行为以源码 `packages/core/src/cli.ts` 与终端输出 `opd --help` 为最终依据；本文档随发版审阅，可能与旧版本有差异。
...
`yarn oc` 等价于 `yarn workspace @opendesktop/core cli`；从 npm 全局安装发行包后命令名为 `**opd**`（例如 `npm install -g @hanzhao111/opendesktop`）。也可用 `**npx @hanzhao111/opendesktop**`。

常用可选参数（与 `opd --help` 文末一致）：
...
```bash
yarn opd -- demo-mock list-window
yarn opd -- demo-mock metrics
yarn opd -- --format json demo-mock snapshot
yarn opd -- --session <session-uuid> demo-mock list-window
```
