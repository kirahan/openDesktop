...
- **WHEN** 客户端调用文档约定的 Agent 动作，提供有效 `targetId` 与合法 `windowMs`，且窗口内至少有一条可观测请求完成
...
系统 SHALL 在聚合结果中提供 **时间窗口内请求总数**；SHALL 提供 **最大并发未完成请求数**（`requestId` 生命周期内 in-flight 峰值的可机读定义由实现文档化）。系统 SHALL 提供 **慢请求** 列表或等价结构：至少包含耗时超过可调阈值（含默认值）的若干条（条数上限文档化），每条含 URL 与耗时。

#### Scenario: 慢请求阈值

- **WHEN** 调用方指定 `slowThresholdMs`（若实现支持）或采用默认阈值
- **THEN** 标记为慢请求的记录 SHALL 仅包含耗时不低于该阈值的请求（在元数据完整的前提下）

### Requirement: 规模与隐私边界

系统 SHALL 对单响应中返回的 URL 条数、单 URL 字符串长度、总 JSON 大小施加上限或截断策略；MAY 对 query string 做剥离或哈希。系统 SHALL 在文档中说明：是否记录 request/response body 本变更 **不要求** MVP 实现。
...
当 Agent API 启用且本能力已实现时，`GET /v1/version` 返回的 `agentActions` SHALL 包含本变更新增的 canonical 动词名（与实现一致）。

#### Scenario: 版本列表包含新动词

- **WHEN** 客户端请求 `GET /v1/version`
- **THEN** `agentActions` 包含网络观测动作的 canonical 名称
