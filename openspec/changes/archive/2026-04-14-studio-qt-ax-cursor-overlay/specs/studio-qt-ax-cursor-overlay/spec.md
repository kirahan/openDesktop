# studio-qt-ax-cursor-overlay（变更增量）
...
系统 SHALL **NOT** 在本能力范围内绘制 **控件矩形边框**或依赖 **AX frame** 几何（该等行为列为非目标，可由后续变更引入）。
...
**Electron 主进程 / preload** SHALL 负责透明覆盖窗口生命周期及向渲染进程暴露 **只读、类型稳定** 的 API（例如启动/停止覆盖、上报指针屏幕坐标）；**packages/web** SHALL 负责会话 UI、节流、发起 Core HTTP 请求与结果展示。**Core** SHALL NOT 为本 MVP 新增专用光标 SSE 端点。
