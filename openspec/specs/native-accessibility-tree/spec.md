...
约定经 Bearer 访问的 **`GET /v1/sessions/:sessionId/native-accessibility-tree`**：在 **macOS** 上对会话子进程 **PID** 枚举 **Accessibility（AX）UI 树** 只读快照；与 **`GET /v1/version` → `capabilities` 中的 `native_accessibility_tree`** 及 Studio「原生无障碍树」入口一致。
...
`GET /v1/version`（或项目统一的 **`capabilities`** 出口）SHALL 能表明 **当前 Core 是否支持** **`native_accessibility_tree`**（在 darwin 上为可用，其它平台为不可用）。客户端 MAY 据此隐藏或禁用相关 UI。
